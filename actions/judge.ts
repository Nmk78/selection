...
export const addRatingsToVotes = async (ratings: Record<string, number>, secretKey: string) => {
  try {
    if (!secretKey) throw new Error("Secret key is required.");

    const specialSecretKey = secretKey.toLowerCase().trim();
    const activeMetadata = await prisma.metadata.findFirst({ where: { active: true } });
    if (!activeMetadata) throw new Error("No active room!");

    const { id: roomId } = activeMetadata;

    const secretKeyRecord = await prisma.specialSecretKey.findUnique({
      where: { specialSecretKey, roomId },
    });

    if (!secretKeyRecord || secretKeyRecord.used) {
      return { success: false, message: "Invalid or already used key." };
    }

    const totalSecretKeys = await prisma.specialSecretKey.count();

    const results: { candidateId: string; message: string; }[] = [];
    const updates = Object.entries(ratings).map(async ([candidateId, rating]) => {
      if (typeof rating !== "number" || rating <= 0) {
        throw new Error(`Invalid rating value for candidate ${candidateId}: ${rating}.`);
      }

      const existingVote = await prisma.vote.findFirst({ where: { candidateId, roomId } });
      if (existingVote) {
        const updatedVote = await prisma.vote.update({
          where: { id: existingVote.id },
          data: { totalRating: { increment: Math.round(rating / (totalSecretKeys / 2)) } },
        });
        results.push({ candidateId, message: `Vote updated for candidate ${candidateId}.` });
      } else {
        const createdVote = await prisma.vote.create({
          data: { candidateId, roomId, totalVotes: 1, totalRating: rating },
        });
        results.push({ candidateId, message: `New vote created for candidate ${candidateId}.` });
      }
    });
    ...
    return { success: true, message: "Ratings successfully added!", details: results };
  } catch (error: any) {
    console.error("Error updating ratings:", error);
    return { success: false, message: "Failed to rate candidates.", error: error.message };
  }
};
